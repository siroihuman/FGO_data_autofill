(function () {
  ...
  const rarityOptions = (selected) => [1, 2, 3, 4, 5].map((rarity) => `<option value="${rarity}"${String(rarity) === String(selected) ? ' selected' : ''}>★${rarity}</option>`).join('');
  ...
  function classGroupHtml(group, groupIndex) {
    ...
    group.skills.forEach((skill, skillIndex) => {
      ...
      html += field('表示名', input(`classGroups.${groupIndex}.skills.${skillIndex}.name`, skill.name));
      html += field('スキルアイコン', input(`classGroups.${groupIndex}.skills.${skillIndex}.icon`, skill.icon));
      ...
      html += `</div>${checkbox(`classGroups.${groupIndex}.skills.${skillIndex}.isNoblePhantasm`, skill.isNoblePhantasm, '宝具：宝具情報テンプレートをフレーバーテキストの先頭に追加')}${checkbox(`classGroups.${groupIndex}.skills.${skillIndex}.rawWiki`, skill.rawWiki, 'フレーバーテキストをWiki記法のまま出力')}</div>`;
    });
  ...
  function ownedSkillHtml(skill, index) {
    ...
    return `<div class="fda-card"><div class="fda-head"><strong>保有スキル ${index + 1}</strong><button type="button" class="fda-btn danger" data-action="delete-owned-skill" data-index="${index}">削除</button></div><div class="fda-grid">
      ${field('見出し番号', input(`ownedSkills.${index}.label`, skill.label))}
    ...
    </div>${checkbox(`ownedSkills.${index}.isNoblePhantasm`, skill.isNoblePhantasm, '宝具：宝具情報テンプレートを解説の先頭に追加')}${checkbox(`ownedSkills.${index}.rawWiki`, skill.rawWiki, '解説をWiki記法のまま出力')}
    ${checkbox(`ownedSkills.${index}.enhancedEnabled`, skill.enhancedEnabled, '強化後データを出力')}
    ${skill.enhancedEnabled ? `<details class="fda-details" open><summary>強化後</summary><div class="fda-grid">${field('強化後スキル名', input(`ownedSkills.${index}.enhanced.name`, e.name))}${field('強化後アイコン', input(`ownedSkills.${index}.enhanced.icon`, e.icon))}${field('強化後解説', textarea(`ownedSkills.${index}.enhanced.description`, e.description), true)}${field('強化後特殊ブロック', textarea(`ownedSkills.${index}.enhanced.rawBlock`, e.rawBlock), true)}</div>${checkbox(`ownedSkills.${index}.enhanced.isNoblePhantasm`, e.isNoblePhantasm, '宝具：宝具情報テンプレートを強化後解説の先頭に追加')}${checkbox(`ownedSkills.${index}.enhanced.rawWiki`, e.rawWiki, '強化後解説をWiki記法のまま出力')}</details>` : ''}</div>`;
  }
  ...
  function render(root, state) {
    root.innerHTML = `<form class="fda"><h2>FGO データオートフィル</h2><div class="fda-version">ver ${VERSION}</div><p class="fda-lead">No・基本情報・パラメーター・クラススキル・保有スキル・宝具・武器を入力し、既存のPukiWikiコードへ反映します。元コードが空欄の場合は新規ページ全体を生成します。</p>
      <section class="fda-sec"><h3>No・基本情報</h3><div class="fda-grid">
        ${field('No.', input('basic.no', state.basic.no))}
        ${field('真名', input('basic.trueName', state.basic.trueName))}
        ${field('レアリティ', `<select data-path="basic.rarity">${rarityOptions(state.basic.rarity)}</select>`)}
        ${field('クラス', `<select data-basic-class>${classOptions(state.basic.className)}</select>`)}
        ${field('性別', input('basic.gender', state.basic.gender))}
        ${field('身長', input('basic.height', state.basic.height))}
        ${field('体重', input('basic.weight', state.basic.weight))}
